...
export const Contact = ({ prefilledScope }: ContactProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Decoupled submission via website inquiry service
    const result = await inquiryService.submitLead(formData);
    ...
    if (result.success) {
      ...
      toast({
        ...
        description: "Your inquiry has been recorded. You can also reach our engineering desk directly via WhatsApp.",
      });
    }
  ...
  const contactInfo = [
    {
      ...
      detail: APP_CONFIG.emails.contact,
      sub: "Average reply time: < 2 hours",
      action: `mailto:${APP_CONFIG.emails.contact}`
    },
    {
      icon: Mail,
      title: "Enterprise Sales",
      detail: APP_CONFIG.emails.sales,
      sub: "For RFPs, contracts & SOWs",
      action: `mailto:${APP_CONFIG.emails.sales}`
    },
    {
      icon: MessageSquare,
      title: "WhatsApp Engineering Desk",
      detail: APP_CONFIG.phone,
      sub: "Instant technical chat support",
      action: `https://wa.me/${APP_CONFIG.whatsappNumber}?text=Hi%20Astitva%20Innovation,%20I%20would%20like%20to%20discuss%20a%20software%20project.`
    },
    {
      icon: Clock,
